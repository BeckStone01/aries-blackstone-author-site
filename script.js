...
document.querySelectorAll('a[href^="#"], a[href$="#signup"]').forEach((link) => {
  link.addEventListener("click", (event) => {
    const linkUrl = new URL(link.href, window.location.href);
    const currentUrl = new URL(window.location.href);

    if (linkUrl.pathname !== currentUrl.pathname || !linkUrl.hash) return;
    if (!scrollToHashTarget(linkUrl.hash)) return;
...
(() => {
  ...
  const heroQuestion = homeHero.querySelector(".hero-ladonna-question");
  if (heroQuestion && heroQuestion.parentElement !== document.body) {
    heroQuestion.classList.add("hero-ladonna-question-fixed");
    document.body.appendChild(heroQuestion);
  }
  ...
  window.addEventListener("scroll", updateHeroFixedState, { passive: true });
  window.addEventListener("resize", updateHeroFixedState);
  window.addEventListener("hashchange", updateHeroFixedState);
})();
